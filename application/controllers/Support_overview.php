<?php
class Support_overview extends CI_Controller{
    protected $_data;
...
//                    print_r($arr_user_id);
        ...
        //hien thi danh sach
        $list_customers = $this->Mcustomers->get_all_customers_filter_support($input);
        $this->_data['list_customers'] = $list_customers;

        //view
        $this->load->view('support_overview/index', $this->_data);
    }
    ...
    public function export(){
        ...
        $this->load->view('support_overview/export', $this->_data);
    }
    ...
    public function detail(){
        ...
        if($sale_id){
            $info = $this->Msupport_detail->get_support_detail($sale_id);
            $this->_data['info'] = $info;

            //hien thi danh sach detail => lay lich su cham soc
            $list_support_detail = $this->Msupport_detail->get_all_support_detail($info['support_id']);
            $this->_data['list_support_detail'] = $list_support_detail;
//            print_r($list_support_detail);
        }
        //========================================== update_customers =================================================

        //========================================== end update_customers =================================================
        //========================================== update_care =================================================
        //cap nhat thong tin update sp detail
...
//            $this->form_validation->set_rules('name_work', 'Tên công việc', 'required');
            if(true){
                ...
                //tien hanh cap nhat
                if($this->Msupport_detail->insert($data_detail)){

                    //redirect
                    redirect(base_url('support_overview/detail/'.$sale_id.'#todolist'));
                }
            }
        ...
        if($this->input->post()){
            ...
            //tien hanh cap nhat
            if($this->Msupport_detail->update_detail($detail_id,$data_detail)){
                //redirect
                return 'success';
            }
        ...
        //view
        $this->load->view('support_overview/detail', $this->_data);
    }
    ...
    public function del_detail(){
        ...
        if($email){
            if($this->input->post()){
                ...
                if($this->Msupport_detail->delete_detail($input)){
                    return 'success';
